	...
	then 
		...
		do
			...
			then 
				...
				if [[ $role = "agent" ]]
				then
					ffmpeg -y -i $file1 -vn -ar 44100 -ac 2 -ab 192 -f mp3 agent_sound.mp3 -loglevel -8
					AGENTFILE=$file1
				else
					CLIENTFILE=$file1
				fi
			...
			then 
				...
				then
					...
					ffmpeg -i agent_sound.mp3 -i "$CLIENTFILE" "$FINALFILE" -loglevel -8
				else
					ffmpeg -y -i $file2 -vn -ar 44100 -ac 2 -ab 192 -f mp3 agent_sound.mp3 -loglevel -8
					ffmpeg -i agent_sound.mp3 -i "$CLIENTFILE" "$FINALFILE" -loglevel -8
					AGENTFILE=$file2
				fi
